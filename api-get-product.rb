...
BEGIN {
  ...
  def show_usage(message, defaults)
    puts "api-get-product.rb: #{message}" if message
    puts <<-EOF

  USAGE:

    api-get-product [OPTIONS] --id=n
  ...
  EXAMPLES:
    ...
    api-get-product --id=11
    api-get-product --id=64 --auth=kiffin:pindakaas
    api-get-product --id=3 --url=www.example.com:8080
